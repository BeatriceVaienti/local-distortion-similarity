...
@dataclass 
class FieldPairOld:
    original_field_1: Field
    original_field_2: Field
    original_map_1: Map = None
    original_map_2: Map = None
    common_extent: tuple = None # (min_x, max_x, min_y, max_y)
    common_alpha_shape_mask : np.array = None
    common_intensity_mask : np.array = None
    area_intensity_mask : float = None
    ...
    def __post_init__(self):
        #we check that the grid size is the same and that the base point is the same as well, otherwise we calculate again the grid for one of the two
        if self.original_field_1.grid_size != self.original_field_2.grid_size:
            if self.original_field_1.base_x != self.original_field_2.base_x or self.original_field_1.base_y != self.original_field_2.base_y:
                print('we need to calculate one of the two maps with the other ones grid size and base point TODO')
            else:
                print('we need to calculate one of the two with the other map gridsize')
        self.find_common_extent(self.original_field_1, self.original_field_2)
        self.get_common_gcp_df()
        self.find_common_extent_gcp_df(self.common_gcp_df_1, self.common_gcp_df_2)
        self.get_cropped_fields()
        self.get_common_alpha_shape_mask()
        self.get_common_intensity_mask()
        self.calculate_cosine_similarity()
    ...
    def find_common_extent(self, field_1, field_2):
        fields = [field_1, field_2]
        min_x_1, max_x_1, min_y_1, max_y_1  = fields[0].base_grid_x.min(), fields[0].base_grid_x.max(), fields[0].base_grid_y.min(), fields[0].base_grid_y.max()
        min_x_2, max_x_2, min_y_2, max_y_2  = fields[1].base_grid_x.min(), fields[1].base_grid_x.max(), fields[1].base_grid_y.min(), fields[1].base_grid_y.max()
    ...
    def crop_to_common_extent(self, field):
        min_x, max_x, min_y, max_y = self.common_extent
        # Generate boolean arrays for rows and columns to keep
        keep_rows = (field.base_grid_y >= min_y) & (field.base_grid_y <= max_y)
        # Columns to keep: X-coordinates within [min_x, max_x]
        keep_columns = (field.base_grid_x >= min_x) & (field.base_grid_x <= max_x)
        # Ensure we're checking along the correct axis for both rows and columns
        keep_rows = np.any(keep_rows, axis=1)  # Any row with Y within bounds
        keep_columns = np.any(keep_columns, axis=0)  # Any column with X within bounds
        # Apply row and column masks to crop
        cropped_base_grid_x = field.base_grid_x[keep_rows][:, keep_columns]
        cropped_base_grid_y = field.base_grid_y[keep_rows][:, keep_columns]
        cropped_displacement_x = field.displacement_x[keep_rows][:, keep_columns]
        cropped_displacement_y = field.displacement_y[keep_rows][:, keep_columns]
        cropped_deformed_x = field.deformed_x[keep_rows][:, keep_columns]
        cropped_deformed_y = field.deformed_y[keep_rows][:, keep_columns]
        cropped_bounds = BoundingBox(left=np.nanmin(cropped_deformed_x), bottom=np.nanmin(cropped_deformed_y), 
                                    right=np.nanmax(cropped_deformed_x), top=np.nanmax(cropped_deformed_y))
        field.bounds = cropped_bounds
        field.base_grid_x = cropped_base_grid_x
        field.base_grid_y = cropped_base_grid_y
        field.displacement_x = cropped_displacement_x
        field.displacement_y = cropped_displacement_y
        field.deformed_x = cropped_deformed_x
        field.deformed_y = cropped_deformed_y
        field.get_alpha_shape_mask()

        return field


    def get_cropped_fields(self):
        cropped_field_1 = Field(self.common_gcp_df_1, self.original_field_1.grid_size, self.original_field_1.base_x, self.original_field_1.base_y)
        cropped_field_2 = Field(self.common_gcp_df_2, self.original_field_2.grid_size, self.original_field_2.base_x, self.original_field_2.base_y)
        self.find_common_extent(cropped_field_1, cropped_field_2)
        
        self.cropped_field_1 = self.crop_to_common_extent(cropped_field_1)
        self.cropped_field_2 = self.crop_to_common_extent(cropped_field_2)
        
    
    def get_common_intensity_mask(self, threshold=0.2):
        #get the cropped and filtered with alpha shape displacement magnitudes for the two field
        magnitudes_1 = np.sqrt(self.cropped_field_1.displacement_x**2 + self.cropped_field_1.displacement_y**2)
        magnitudes_2 = np.sqrt(self.cropped_field_2.displacement_x**2 + self.cropped_field_2.displacement_y**2)
        #filter the magnitudes using the alpha shape mask
        magnitudes_1[~self.cropped_field_1.alpha_shape_mask] = np.nan
        magnitudes_2[~self.cropped_field_2.alpha_shape_mask] = np.nan
        quartile_1 = np.nanquantile(magnitudes_1, threshold)
        quartile_2 = np.nanquantile(magnitudes_2, threshold)
        intensity_mask_1 = magnitudes_1 >= quartile_1
        self.cropped_field_1.intensity_mask = intensity_mask_1
...
class FieldOld:
    ...
    real_points: np.array = None
    pixel_points: np.array = None
    affine_aligned_pixel_points: np.array = None
    bounds: tuple = None
    alpha_shape: Polygon = None
    alpha_shape_mask : np.array = None
    base_grid_x: np.array = None
    ...
    def __post_init__(self):
        ...
        if self.displacement_x is None:
            ...
            transform = estimate_transform('polynomial', pixel_points, real_points, 2) 
            affine_aligned_pixel_points = transform(pixel_points) 
            self.affine_aligned_pixel_points = affine_aligned_pixel_points
...
##the following functions are necessary to create the vector field and called by the __post_init___
    def get_real_points(self):
        """gets the real points from the mapX and mapY columns of the df"""
        if self.real_points is None:
            real_points = np.array(self.gcp_df[['mapX', 'mapY']])
    ...
    def get_pixel_points(self):
        """gets the pixel points from sourceX and sourceY columns of the df"""
        if self.pixel_points is None:
            pixel_points = np.array(self.gcp_df[['sourceX', 'sourceY']])
            self.pixel_points = pixel_points
        else:
            pixel_points = self.pixel_points
        return pixel_points
    
    def get_gcp_list(self): #probably not needed, could be eliminated #todo
        gcp_list = create_gcp_list_from_df(self.gcp_df)
        self.gcp_list = gcp_list
        return gcp_list
    
    def get_alpha_shape(self, alpha=0.001):
        """Gets the alpha shape of the affine_aligned_pixel_points."""
        if self.affine_aligned_pixel_points is not None:
            self.alpha_shape = find_alpha_shape(self.real_points, alpha)
            return self.alpha_shape
        else:
            print("Affine aligned pixel points are not set.")
            return None
    def get_alpha_shape_mask(self):
        base_grid_x, base_grid_y = self.base_grid_x, self.base_grid_y 
        mask_inside_alpha_shape = np.zeros_like(base_grid_x, dtype=bool)
        for i in range(base_grid_x.shape[0]):
            for j in range(base_grid_x.shape[1]):
                ...
                if self.alpha_shape.contains(point):
                    mask_inside_alpha_shape[i, j] = True
        self.alpha_shape_mask = mask_inside_alpha_shape
        return mask_inside_alpha_shape

    def get_grid(self):
        if self.base_grid_x is None:
            base_grid_x, base_grid_y = create_grid(self.bounds, self.base_x, self.base_y, self.grid_size)
            self.base_grid_x, self.base_grid_y = base_grid_x, base_grid_y
            # Initialize masks for points inside the alpha shape
            mask_inside_alpha_shape = np.zeros_like(base_grid_x, dtype=bool)
            # Initialize the filtered grids as copies of the original grids
            ...
            self.alpha_shape_mask = mask_inside_alpha_shape
            # Substitute NaN for points outside the alpha shape

    def get_deformed_grid(self):
        if self.deformed_x is None:
            self.displacement_x, self.displacement_y = create_displacement_vectors(self.affine_aligned_pixel_points, self.real_points, self.base_grid_x, self.base_grid_y)
            deformed_x = self.base_grid_x + self.displacement_x
            deformed_y = self.base_grid_y + self.displacement_y 
            self.deformed_x, self.deformed_y = deformed_x, deformed_y

    def get_intensity_mask(self, intensity_threshold=0.1):
        displacement_x_filtered = np.copy(self.displacement_x)
        displacement_y_filtered = np.copy(self.displacement_y)
        mask = self.alpha_shape_mask
        displacement_x_filtered[~mask] = np.nan
        displacement_y_filtered[~mask] = np.nan
        magnitudes = np.sqrt(displacement_x_filtered**2 + displacement_y_filtered**2) #calculated over the points in the alpha shape
        quartile = np.nanquantile(magnitudes, intensity_threshold)
    ...
    def plot_affine_transformation(self):
        fig, ax = plt.subplots(figsize=(10,10))
        plt.scatter(self.get_real_points()[:,0], self.get_real_points()[:,1], c='r', label='real points')
        plt.scatter(self.affine_aligned_pixel_points[:,0], (self.affine_aligned_pixel_points)[:,1], c='b', label='transformed points (affine)')
        ax.legend()
        plt.show()

    def plot_alpha_shape(self):
        """Plots the alpha shape of the affine_aligned_pixel_points."""
        alpha_shape = self.alpha_shape
        if alpha_shape is not None:
            x,y = alpha_shape.exterior.xy
            plt.figure()
    ...
    def plot_vector_field_and_affine_transformation(self):
        fig, ax = plt.subplots(figsize=(10,10))
        plt.scatter(self.real_points[:,0], self.real_points[:,1], c='r', label='real points')
        plt.scatter(self.affine_aligned_pixel_points[:,0], (self.affine_aligned_pixel_points)[:,1], c='b', label='transformed points (affine)')
        
        base_grid_x, base_grid_y = self.base_grid_x, self.base_grid_y
        #plot a grid that has the starting point in base_grid_x, base_grid_y and the displacement in displacement_x, displacement_y
        X_component, Y_component = self.displacement_x, self.displacement_y
        X, Y = np.meshgrid(base_grid_x, base_grid_y)
...
class GraphPairOld:
    ...
    #filter_1_graph : nx.Graph = None
    #filter_2_graph : nx.Graph = None
    common_combined_mask : np.array = None
    base_grid_nodes_edges : tuple = None #new replacing base_grid_graph
    deformed_grid_1_nodes_edges : tuple = None #new replacing deformed_grid_1_graph
    deformed_grid_2_nodes_edges : tuple = None #new replacing deformed_grid_2_graph
    #instead of creating an actual graph we could create a dictionary with the nodes and the edges and then create the graph when needed
    #bc the structure of the graph is always the same
    #the graph itself is needed to find the seeds. we may even just create subgraphs?
    def __post_init__(self):
        self.get_cropped_grids()
        #self.project_common_grid()  # this is not needed, unless we want to have the grid in the epsg 4362 for folium
        ...
        if True:
            print(f"Time elapsed get_base_grid_graph: {end - start} seconds")
        ...
        if True:
            print(f"Time elapsed get_deformed_grid_graph: {end - start} seconds")
        
        start = time.time()
        #self.get_ratio_graph(get_filters = True) # for the moment we don't calculate the filters because it takes too much time
        self.get_ratio_graph_from_dicts(get_filters=True)
        end = time.time()
        if True:
            print(f"Time elapsed get_ratio_graph: {end - start} seconds")
    ...
    def get_cropped_grids(self):
        self.cropped_grid_1 = self.map_pair.cropped_grid_1
        self.cropped_grid_2 = self.map_pair.cropped_grid_2
    ...
    def project_common_grid_to_map_space(self):
        # Apply RBF to the local EPSG grid for both maps
        common_grid_local_epsg = np.stack((self.cropped_grid_1.base_grid_x, self.cropped_grid_1.base_grid_y), axis=-1)
        self.common_grid_map_1 = self.apply_rbf_to_grid(self.cropped_grid_1.gcp_df, common_grid_local_epsg)
        self.common_grid_map_2 = self.apply_rbf_to_grid(self.cropped_grid_2.gcp_df, common_grid_local_epsg)
    ...
    def get_base_grid_graph(self):
        #base grid graph
        grid = np.stack((self.cropped_grid_1.base_grid_x, self.cropped_grid_1.base_grid_y), axis=-1)
        G = self.create_grid_graph(grid)
        self.base_grid_graph = G
        return G
    ...
    def get_deformed_grid_graph(self):
        #deformed grid graph
        grid_1_graph = self.create_grid_graph(self.common_grid_map_1)
        grid_2_graph = self.create_grid_graph(self.common_grid_map_2)
        self.deformed_grid_1_graph = grid_1_graph
        self.deformed_grid_2_graph = grid_2_graph
        return grid_1_graph, grid_2_graph
    ...
    def create_grid_nodes_edges(self, grid):
        flat_grid = grid.reshape(-1, 2)
        rows, cols = self.cropped_grid_1.base_grid_x.shape
        nodes = {idx: {"pos": point} for idx, point in enumerate(flat_grid)}
        edges = {}
    ...
    def create_grid_graph(self, grid):
        ...
        # Prepare to add edges using bulk operations
        edges = []
        for row in range(rows):
            for col in range(cols):
                idx = row * cols + col
                ...
                if col < cols - 1:
                    right_neighbor_idx = idx + 1
                    edges.append((idx, right_neighbor_idx))
                # Bottom neighbor
                if row < rows - 1:
                    bottom_neighbor_idx = idx + cols
                    edges.append((idx, bottom_neighbor_idx))
                # Diagonal neighbors
                if row < rows - 1 and col < cols - 1:
                    diagonal_neighbor_idx_br = idx + cols + 1
                    edges.append((idx, diagonal_neighbor_idx_br))
                if row < rows - 1 and col > 0:
                    diagonal_neighbor_idx_bl = idx + cols - 1
                    edges.append((idx, diagonal_neighbor_idx_bl))

        # Calculate distances and add all edges with attributes at once
        edge_lengths = [np.linalg.norm(flat_grid[u] - flat_grid[v]) for u, v in edges]
        G.add_edges_from((u, v, {'length': length}) for (u, v), length in zip(edges, edge_lengths))

        return G
    
    def convert_dicts_to_graph(self, ratios, seeds):
        G = nx.Graph()
        for edge, ratio in ratios.items():
            G.add_edge(*edge, ratio=ratio)
        for node, seed in seeds.items():
    ...
    def create_ratio_graph_from_dicts(self, deformed_edges_1, deformed_edges_2, seed_threshold=0.1):
        ...
        # Potentially return a NetworkX graph if needed for further operations
        return self.convert_dicts_to_graph(ratios, seeds)
    ...
    def create_ratio_graph(self, G_deformed_1, G_deformed_2, seed_threshold=0.1):
        ...
        G_ratios = nx.Graph()
        self.preprocess_graph_lengths(G_deformed_1, G_deformed_2)
        edge_lengths_1 = np.array([(u, v, data['scaled_length']) for u, v, data in G_deformed_1.edges(data=True) if G_deformed_2.has_edge(u, v)])
        edge_lengths_2 = np.array([(u, v, G_deformed_2[u][v]['scaled_length']) for u, v, _ in edge_lengths_1])

        # Calculate ratios using vectorized operations
        valid_indices = edge_lengths_2[:, 2] != 0
        ratios = np.where(valid_indices, edge_lengths_1[:, 2] / edge_lengths_2[:, 2], edge_lengths_1[:, 2] / 1e-6)

        # Create the graph of ratios
        G_ratios = nx.Graph()
        for (u, v), ratio in zip(edge_lengths_1[:, :2], ratios):
            G_ratios.add_edge(u, v, ratio=ratio)

        # Node processing
        for node in G_ratios.nodes():
            # Get all connected edges and their ratios directly
            neighbor_ratios = [G_ratios[u][v]['ratio'] for u, v in G_ratios.edges(node)]
            if neighbor_ratios:
                max_ratio = max(neighbor_ratios)
                min_ratio = min(neighbor_ratios)
                delta = max_ratio - min_ratio

                # Determine if the node should be a seed
                seed = 1 if delta < seed_threshold else 0
                G_ratios.nodes[node]['seed'] = seed
        return G_ratios
    ...
    def get_ratio_graph_from_dicts(self, get_filters=False):
        # Unpack edges from the tuples
        _, edges_base = self.base_grid_nodes_edges
        _, edges_deformed_1 = self.deformed_grid_1_nodes_edges
        _, edges_deformed_2 = self.deformed_grid_2_nodes_edges
        ...
        # Create ratio graphs using the preprocessed and scaled edges
        self.ratio_graph = self.create_ratio_graph_from_dicts(edges_deformed_1, edges_deformed_2, seed_threshold=0.2)
        if get_filters:
            self.filter_1_graph = self.create_ratio_graph_from_dicts(edges_base, edges_deformed_1, seed_threshold=0.02)
            self.filter_2_graph = self.create_ratio_graph_from_dicts(edges_base, edges_deformed_2, seed_threshold=0.02)
    ...
    def get_combined_mask(self):
        ...
        def update_mask_from_seeds(G_ratios_filter):
            for node, data in G_ratios_filter.nodes(data=True):
                if data.get('seed', 0) == 1:  # Check if the node is marked as a seed
                    # Ensuring node is used as an integer for indexing
                    node_index = int(node)
                    row = node_index // cols
                    col = node_index % cols
                    if 0 <= row < rows and 0 <= col < cols:
        ...
        update_mask_from_seeds(G_ratios_filter_2)
        self.common_combined_mask = new_mask
        self.filter_1_graph = None
        self.filter_2_graph = None
        self.base_grid_graph = None
        return new_mask
    ...
    def plot_graph_on_image_mask_filtered(self, map = '1', mask_intensity = True, save = True, folder_path = './results' ):
        ...
        if map == '1':
            ...
            G_deformed = self.deformed_grid_1_graph
            flat_grid = self.common_grid_map_1.reshape(-1, 2)
            name = self.map_pair.original_map_1.name

        elif map == '2':
            image_path = self.map_pair.original_map_2.image_path
            G_deformed = self.deformed_grid_2_graph
            flat_grid = self.common_grid_map_2.reshape(-1, 2)
            name = self.map_pair.original_map_2.name
        image = cv2.imread(image_path)
        image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        save_name =   '/'+ name +'_filtered' '.png'
        # Create the folder if it does not exist
        ...
        # Convert node indices to grid position and check against the alpha_shape_mask
        filtered_nodes = {idx for idx in range(rows * cols) if mask[idx // cols, idx % cols]}
        filtered_edges = [(u, v) for u, v in G_ratios.edges() if u in filtered_nodes and v in filtered_nodes]
    ...
    def project_common_grid(self, src_epsg=28193, dst_epsg=4326): #todo substitute the src_epsg so that it will be determined by the map
        base_grid_x = self.cropped_grid_1.base_grid_x
        base_grid_y = self.cropped_grid_1.base_grid_y
        transformer = Transformer.from_crs(f"EPSG:{src_epsg}", f"EPSG:{dst_epsg}", always_xy=True)
        ...
        self.common_grid_epsg_4326 = np.stack((projected_x, projected_y), axis=-1)
        self.map_center_epsg_4326 = np.mean(projected_y), np.mean(projected_x)
    def scale_edge_lengths_rx(self, G, scale_factor):
        for u, v in G.edge_list():
            # Fetch existing edge data
            data = G.get_edge_data(u, v)
            # Scale the 'length' attribute
            data['scaled_length'] = data['length'] * scale_factor
            # Update the edge with new data
            G.update_edge(u, v, data)

    def calculate_median_length_rx(self, G):
        lengths = [G.get_edge_data(u, v)['length'] for u, v in G.edge_list()] 
        return np.median(lengths)
   
    def preprocess_graph_lengths_rx(self, G_deformed_1, G_deformed_2):
        combined_median = self.calculate_combined_median_length(G_deformed_1, G_deformed_2)
        median_1 = self.calculate_median_length(G_deformed_1)
        median_2 = self.calculate_median_length(G_deformed_2)
    ...
    def create_grid_graph_rx(self, grid):
        G = rx.PyGraph()  # Create an undirected graph
        flat_grid = grid.reshape(-1, 2)
        rows, cols = self.cropped_grid_1.base_grid_x.shape

        # Add nodes with positions using bulk operation
        nodes_with_positions = [G.add_node({"pos": point}) for point in flat_grid]
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if col < cols - 1:
                    right_neighbor_idx = idx + 1
                    edge_length = np.linalg.norm(flat_grid[idx] - flat_grid[right_neighbor_idx])
                    G.add_edge(nodes_with_positions[idx], nodes_with_positions[right_neighbor_idx], {'length': edge_length})
                # Bottom neighbor
                if row < rows - 1:
                    bottom_neighbor_idx = idx + cols
                    edge_length = np.linalg.norm(flat_grid[idx] - flat_grid[bottom_neighbor_idx])
                    G.add_edge(nodes_with_positions[idx], nodes_with_positions[bottom_neighbor_idx], {'length': edge_length})
                # Diagonal neighbors
                if row < rows - 1 and col < cols - 1:
                    diagonal_neighbor_idx_br = idx + cols + 1
                    edge_length = np.linalg.norm(flat_grid[idx] - flat_grid[diagonal_neighbor_idx_br])
                    G.add_edge(nodes_with_positions[idx], nodes_with_positions[diagonal_neighbor_idx_br], {'length': edge_length})
                if row < rows - 1 and col > 0:
                    diagonal_neighbor_idx_bl = idx + cols - 1
                    edge_length = np.linalg.norm(flat_grid[idx] - flat_grid[diagonal_neighbor_idx_bl])
                    G.add_edge(nodes_with_positions[idx], nodes_with_positions[diagonal_neighbor_idx_bl], {'length': edge_length})
                    
        return G

    def calculate_combined_median_length_rx(self, G_deformed_1, G_deformed_2):
        median_1 = self.calculate_median_length(G_deformed_1)
        median_2 = self.calculate_median_length(G_deformed_2)
        return np.mean([median_1, median_2])

    def create_ratio_graph_rx(self, G_deformed_1, G_deformed_2, seed_threshold=0.1):
        self.preprocess_graph_lengths(G_deformed_1, G_deformed_2)
        G_ratios = rx.PyGraph()

        # Ensure all nodes are added first
        nodes_in_deformed_1 = set(G_deformed_1.node_indexes())
        nodes_in_deformed_2 = set(G_deformed_2.node_indexes())
        common_nodes = nodes_in_deformed_1.intersection(nodes_in_deformed_2)

        # Add nodes to G_ratios
        for node in common_nodes:
            G_ratios.add_node(node)

        # Now add edges with their calculated ratios
        for edge in G_deformed_1.edge_list():
            u, v = edge
            if G_deformed_2.has_edge(u, v) and u in common_nodes and v in common_nodes:
                # Get the edge data from both graphs
                data_1 = G_deformed_1.get_edge_data(u, v)
                data_2 = G_deformed_2.get_edge_data(u, v)
                
                length_1 = data_1['scaled_length']
                length_2 = data_2['scaled_length']
                ratio = length_1 / length_2 if length_2 != 0 else length_1 / 1e-6
                
                # Add the edge with the ratio as the edge data to the ratios graph
                G_ratios.add_edge(u, v, {'ratio': ratio})
        self.check_for_seeds(G_ratios, seed_threshold)
        return G_ratios

    def get_combined_mask_rx(self):
        alpha_shape_mask = self.map_pair.common_alpha_shape_mask
        G_ratios_filter_1 = self.filter_1_graph
        G_ratios_filter_2 = self.filter_2_graph
        rows, cols = self.cropped_grid_1.base_grid_x.shape
        ...
        # Determine seed nodes from both graphs and set their corresponding mask values to False
        def update_mask_from_seeds(G_ratios_filter):
            for node in G_ratios_filter.node_indexes():
                # Fetch node data safely
                data = G_ratios_filter[node]
                # Check if data is a dictionary and get the 'seed' value, else default to 0
                seed_value = data.get('seed', 0) if isinstance(data, dict) else 0
                
                if seed_value == 1:  # Check if the node is marked as a seed
                    node_index = int(node)  # Ensuring node is used as an integer for indexing
                    row = node_index // cols
                    col = node_index % cols
                    if 0 <= row < rows and 0 <= col < cols:
                        new_mask[row, col] = False
    ...
    def plot_graph_on_image_mask_filtered_rx(self, map = '1', mask_intensity = True, save = True, folder_path = './results' ):
        # Load and prepare the map image
        G_ratios = self.ratio_graph
        if G_ratios:  # Ensure there is at least one node
            some_node = next(iter(G_ratios.node_indexes()))  # Get the first node
            G_ratios[some_node] = {'seed': 1}  # Manually set as seed

        seed_nodes = [node for node in filtered_nodes if isinstance(G_ratios[node], dict) and G_ratios[node].get('seed', 0) == 1]
        if seed_nodes:
            print("Seed nodes identified:", seed_nodes)
        else:
        ...
        # Filter nodes and edges, plot them
        filtered_nodes = {idx for idx in range(rows * cols) if mask[idx // cols, idx % cols]}
        filtered_edges = [(u, v) for u, v in G_deformed.edges() if u in filtered_nodes and v in filtered_nodes]
        for u, v in filtered_edges:
            x = [flat_grid[u][0], flat_grid[v][0]]
        ...
        # Handling seed nodes and their positions
        seed_nodes = [node for node in filtered_nodes if isinstance(G_ratios[node], dict) and G_ratios[node].get('seed', 0) == 1]
        if seed_nodes:  # Check if there are any seed nodes
            seed_positions = np.array([flat_grid[node] for node in seed_nodes])
            if seed_positions.ndim == 1:  # If only one seed node, reshape array
                seed_positions = seed_positions.reshape(1, -1)
            plt.scatter(seed_positions[:, 0], seed_positions[:, 1], color='red', s=15, label='Seed Nodes', zorder=5, alpha=0.5)
        else:
            print("No seed nodes found")
